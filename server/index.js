...
app.get("/", (req, res) => res.send("Welcome to semicolon academy APIs, use /video-request to get data"));
...
app.get("/video-request", async (req, res, next) => {
  const { sortBy, searchKey } = req.query;
  let data;

  if (searchKey) {
    data = await VideoRequestData.searchRequests(searchKey);
  } else {
